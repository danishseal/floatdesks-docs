...
Everything else the essay promised is built and is the same: one real share per
...
Five launches have run end to end, each a meme quoted in the fSHARE it settles
...
2. **One key owns all of it**, the breaker included, and it is also the sole
   ...
   without touching any other contract. They have been: `posterCount` now
...
9. **Quote-pool depth is a band, not a hook.** Roughly plus or minus 2%, and a
   ...
   another branch. `Registry.addrs("DESK_HOOK")` is the zero address. It is
